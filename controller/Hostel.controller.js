...
const createHostel = async (req, res) => {
  try {
    ...
    return res.status(201).json({ message: 'Hostel created successfully', hostel: newHostel });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: 'Error creating hostel', error: error.message });
  }
};
...
const getAllHostels = async (req, res) => {
  try {
    ...
    console.error(error);
    return res.status(500).json({ message: 'Error fetching hostels', error: error.message });
  }
};
...
// 3. Get a single hostel by its ID
 const getHostelById = async (req, res) => {
  try {
    ...
    console.error(error);
    return res.status(500).json({ message: 'Error fetching hostel', error: error.message });
  }
};
...
const updateHostel = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: 'Hostel updated successfully', hostel });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: 'Error updating hostel', error: error.message });
  }
};
...
const deleteHostel = async (req, res) => {
  try {
    ...
    if (!hostel || hostel.deleteFlag) {
      return res.status(404).json({ message: 'Hostel not found or already deleted' });
    }

    hostel.deleteFlag = true; // Mark the hostel as deleted
    await hostel.save();

    return res.status(200).json({ message: 'Hostel marked for deletion', hostel });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: 'Error deleting hostel', error: error.message });
  }
};
